...
public class ReviewsCursorResponse {
    ...
    public static ReviewsCursorResponse of(@NotNull ReviewCursorPaging reviewCursorPaging, @NotNull StoreDictionary storeDictionary, @NotNull User user) {
        List<ReviewDetailResponse> reviews = combineReviewDetailResponse(reviewCursorPaging.getCurrentCursorItems(), storeDictionary, user);
        if (reviewCursorPaging.hasNext()) {
            return new ReviewsCursorResponse(reviews, reviewCursorPaging.getNextCursor().getId());
        }
